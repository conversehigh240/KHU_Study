...
int IndexIs(VertexType* vertices, VertexType fromVertex);
...
int IndexIs(VertexType* vertices, VertexType fromVertex)
{
	...
	while (!(fromVertex == vertices[index]))
		index++;
...
/* 7-a
General case는 startVertex가 인접노드를 따라서 재귀호출을 진행하도록 하게끔 하는 case이다.
Base case는 startVertex가 인접노드를 따라서 재귀호출을 진행할 때 endVertex와 같은 값이 되는 case이다.
*/